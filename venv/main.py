...
def main():
    ...
    if args.input == None or args.output == None:
        ...
        os._exit(0)
    #В задании не указано какой тип пути будет указан до файта. Так что берем абсолютный.
    if os.path.isabs(args.input) == False:
        args.input = os.path.abspath(args.input)
    if os.path.isabs(args.output) == False:
        args.output = os.path.dirname(args.input) +'\\' + args.output
